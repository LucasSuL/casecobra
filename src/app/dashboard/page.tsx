...
const Page = async () => {
  ...
  const orders = await db.order.findMany({
    where: {
      isPaid: true,
      createdAt: {
        gte: new Date(new Date().setDate(new Date().getDate() - 7)), // find orders last week
      },
    },
    orderBy: {
      createdAt: "desc", // newest order show first
    },
    include: {
      // get user and shipaddr info from DB
      user: true,
      shippingAddress: true,
    },
  });

  // calc the overall amount paid (turn over) in last week
